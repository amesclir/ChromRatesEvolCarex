...
result_noNZ <- phylo_path(models, data = my_data3, tree = my_tree, model = 'lambda', na.rm = TRUE)

result_noNZ
(snoNZ <- summary(result_noNZ))
snoNZ
plot(snoNZ)
average_model_full_noNZ <- average(result_noNZ, avg_method = "full")

average_model_full_noNZ
```
...
VIGNEA
tipstoremove <- setdiff(my_tree$tip.label, VIGNEA)
my_tree_vignea <- drop.tip(my_tree, tipstoremove)
my_data_vignea <- my_data2[my_data2$species %in% VIGNEA,]

models_vignea <- define_model_set(
  null = c(),
...
result_vignea <- phylo_path(models_vignea, data = my_data_vignea, tree = my_tree_vignea, model = 'lambda', na.rm = TRUE)

result_vignea

(svignea <- summary(result_vignea))


result_vignea_norates <- phylo_path(modelsnorates, data = my_data_vignea, tree = my_tree_vignea, model = 'lambda', na.rm = TRUE)
(svigneanorates <- summary(result_vignea_norates))
plot(svigneanorates)

(best_model_vignea_norates <- best(result_vignea_norates))


plot(best_model_vignea_norates)



UNISPICATESCHOENOXIPHIUM
tipstoremove <- setdiff(my_tree$tip.label, UNISPICATESCHOENOXIPHIUM)
my_tree_us <- drop.tip(my_tree, tipstoremove)
my_data_us <- my_data2[my_data2$species %in% UNISPICATESCHOENOXIPHIUM,]

models_us <- define_model_set(
  null = c(),
...
result_us <- phylo_path(models_us, data = my_data_us, tree = my_tree_us, model = 'lambda', na.rm = TRUE)

#ERRORS!



result_us_norates <- phylo_path(modelsnorates, data = my_data_us, tree = my_tree_us, model = 'lambda', na.rm = TRUE)

#ERRORS!!!


CAREX
tipstoremove <- setdiff(my_tree$tip.label, CAREX)
my_tree_carex <- drop.tip(my_tree, tipstoremove)
my_data_carex <- my_data2[my_data2$species %in% CAREX,]
my_data_carex$DI[my_data_carex$DI > 3] <- NA

result_carex <- phylo_path(models_vignea, data = my_data_carex, tree = my_tree_carex, model = 'lambda', na.rm = TRUE)

(scarex <- summary(result_carex))
plot(scarex)

(best_model_carex <- best(result_carex))


plot(best_model_carex)




result_carex_norates <- phylo_path(modelsnorates, data = my_data_carex, tree = my_tree_carex, model = 'lambda', na.rm = TRUE)
(scarex_norates <- summary(result_carex_norates))
plot(scarex_norates)

(best_model_carex_norates <- best(result_carex_norates))


plot(best_model_carex_norates)
